...
export const VolumeCard: React.FC<VolumeCardProps> = ({ volume, onClick }) => {
  ...
  const badgeVariants: Record<RhcaVolume["availability"], string> = {
    ...
    coming_soon: "bg-blue-100 text-foreground/80",
  };
      ...
      >
        <div className="flex flex-col gap-4 p-4">
          ...
          <div className="space-y-1">
            ...
            {specialty && (
              <p className="text-sm text-muted-foreground">Spécialité: {specialty}</p>
            )}
            {category && (
              <p className="text-sm text-muted-foreground">Catégorie: {category}</p>
            )}
          </div>
        </div>
      </Card>
